#!/bin/bash
clear
echo "[==>  Script ArchLinux Post Installation  <==]"
echo "                Por Krafairus"
echo ""
echo "[==>  Actualizando El Sistema  <==]"
sudo pacman -Syuu --noconfirm
echo "[==>  Instalando Los Paquetes De Mate Desktop Environment  <==]"
sudo pacman -S --noconfirm --needed xorg xorg-server 
sudo pacman -S --noconfirm --needed mate mate-extra ttf-freefont lightdm lightdm-gtk-greeter xscreensaver
echo "[==>  Activando Servicios De Display Manager Y NetWork Manager  <==]"
sudo systemctl enable NetworkManager.service
sudo systemctl enable lightdm.service
./options/desktop/systemop.sh
